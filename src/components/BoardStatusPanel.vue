...
const filteredCapturedPieces = computed(() => {
  ...
  if (props.currentMoveIndex >= 0) {
    return sideFilteredPieces.filter(piece => piece.moveNumber <= props.currentMoveIndex + 1);
...
const materialAdvantage = computed(() => {
  ...
  if (props.currentMoveIndex >= 0) {
    visibleCapturedPieces = visibleCapturedPieces.filter(piece => 
      piece.moveNumber <= props.currentMoveIndex + 1
    );
  } else if (props.currentMoveIndex === -2) {
